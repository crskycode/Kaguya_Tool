...
namespace Params_Tool
{
    ...
    class GameScriptPattern
    {
        ...
        class Struct14Collection
        {
            List<Struct14> _collection;

            public void Deserialize(BinaryReader reader, double version)
            {
                var count = reader.ReadInt32();

                _collection = new List<Struct14>(count);

                for (var i = 0; i < count; i++)
                {
                    var item = new Struct14();
                    item.Deserialize(reader, version);
                    _collection.Add(item);
                }
            }
        ...
        {
            List<Group> _collection;

            public void Deserialize(BinaryReader reader, double version)
            {
                var count = reader.ReadInt32();

                _collection = new List<Group>(count);
        ...
        public void Deserialize(BinaryReader reader, double version)
        {
            ...
            if (version >= 5.7)
            {
                ...
                _items.Deserialize(reader, version);
            }

            field_14 = new Struct14Collection();
            field_14.Deserialize(reader, version);
